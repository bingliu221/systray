...
gboolean do_add_or_update_menu_item(gpointer data) {
    ...
    for (it = global_menu_items; it != NULL; it = it->next) {
        ...
        if (item->menu_id == mii->menu_id) {
            ...
            if (mii->isCheckable) {
                // We need to block the "activate" event, to emulate the same
                // behaviour as in the windows version A Check/Uncheck does
                // change the checkbox, but does not trigger the checkbox
                // menuItem channel
                g_signal_handler_block(GTK_CHECK_MENU_ITEM(item->menu_item),
                                       item->signalHandlerId);
                gtk_check_menu_item_set_active(
                    GTK_CHECK_MENU_ITEM(item->menu_item), mii->checked == 1);
...
gboolean do_quit(gpointer data) {
    ...
    app_indicator_set_status(global_app_indicator,
